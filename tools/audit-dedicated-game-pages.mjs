...
assert.equal(wrangler.includes('"/play/'),false,'daily game documents must bypass the Worker cold-start path');
for(const [game,path] of Object.entries(routes)){
  const staticPage=read('./public'+path+'index.html');
  assert.ok(staticPage.includes('data-game-page="'+game+'"'),`static shell is missing game marker for ${game}`);
  assert.ok(staticPage.includes('/app-core.js?v=dedicated-7')&&staticPage.includes('/game-page.js?v=2'),`static shell is missing shared runtime for ${game}`);
  assert.ok(staticPage.includes('finish all seven games.'),`static shell contains stale game-count copy for ${game}`);
  assert.ok(sw.includes("'"+path+"'"),`service worker does not install ${game}`);
}
assert.ok(sw.includes("const staticGamePath")&&sw.includes("const cached = await cache.match(staticGamePath)"),'daily game navigation must be cache-first');
...
assert.ok(Number(sw.match(/clue-morning-pwa-v(\d+)/)?.[1]||0) >=57,'PWA cache must be bumped for the all-game offline bundle');
