...
const coreSource=fs.readFileSync('./public/app-core.js','utf8');const appSource=fs.readFileSync('./public/app.js','utf8')+coreSource;assert.ok(coreSource.includes("['QWERTYUIOP','ASDFGHJKL','ZXCVBNM']")&&coreSource.includes("row.className='key-row'"),'Letter Grid keyboard should use the approved three-row QWERTY structure');assert.ok(coreSource.includes("document.querySelectorAll('.tab[data-tab]').forEach"),'In-document navigation must bind only real panel tabs');assert.ok(coreSource.includes('document.querySelectorAll(".panel").forEach'),'Tab navigation must iterate all panels with querySelectorAll');const indexSource=fs.readFileSync('./public/index.html','utf8');assert.ok(indexSource.includes('href="/play/letter-trail/"')&&indexSource.includes('href="/play/deep-cut/"'),'Homepage daily cards must use dedicated play routes');assert.ok(appSource.includes('Keep guessing — the clock is still running.'),'Deep Cut invalid guesses should preserve the active prompt');assert.ok(appSource.includes('startUnlimitedGame'),'Unlimited library UI should be wired');assert.ok(appSource.includes('Play More Games')&&appSource.includes('Share Score'),'Daily completion score cards should offer retention and sharing actions');
