(()=>{
  ...
  const EXP_STORE='clue-morning-daily-expansion-v1';
  ...
  const BOARDS=[
    ...
    {group:'Daily games',items:[['grid','Letter Grid'],['groups','Four Groups'],['trail','Letter Trail'],['link','Triple Link'],['steps','Word Steps'],['deepcut','Deep Cut'],['situation','Situation'],['lastcall','Last Call']]},
  ...
  async function request(path,options={}){const r=await fetch(path,{cache:'no-store',...options});const j=await r.json();if(!r.ok)throw new Error(j.error||'Request failed');return j}
  ...
  function installPanel(){
    ...
    const list=$('#leaderboardList');if(list)list.insertAdjacentHTML('beforebegin','<div id="leaderBoardContext" class="leaderboards-v2-note"></div>');
  }
  ...
  async function saveName(){
    ...
    try{await postJson('/api/leaderboard/name',{playerId:playerId(),name});if(msg){msg.className='message good';msg.textContent=`Saved as ${name}.`}const placeholder=$('#leaderPlaceholderName');if(placeholder)placeholder.textContent=fallbackName();posted.clear();syncDailyScores(true);loadBoard()}
  ...
  function boardContext(){
    if(board==='today')return 'Completed core sets rank by today’s total, with Situation and Last Call added as they are finished.';
    if(board==='all')return 'All-time total points across completed daily sets.';
  ...
  function renderRows(rows){
    ...
    rows.forEach((row,i)=>{const el=document.createElement('div');el.className='leader-row';el.innerHTML=`<span class="leader-rank">${i+1}</span><div><div class="leader-name">${esc(row.display_name)}</div><div class="leader-sub">${esc(subFor(row))}</div></div><div class="leader-score">${Number(row.score||0).toLocaleString()}</div>`;list.appendChild(el)})
  }
  async function loadBoard(){
    ...
    try{const suffix=date?`&date=${encodeURIComponent(date)}`:'';const j=await request(`/api/leaderboard?board=${encodeURIComponent(board)}${suffix}`);if(!j.enabled){if(list)list.innerHTML='<div class="empty-state">Leaderboard database is not enabled yet.</div>';return}renderRows(j.rows||[])}catch(err){if(list)list.innerHTML=`<div class="empty-state">${esc(err.message)}</div>`}finally{loading=false}
  }
  ...
  function syncDailyScores(force=false){
    if(syncQueued&&!force)return;syncQueued=true;queueMicrotask(()=>{syncQueued=false;if(!date)return;
      ...
      const exp=readStore(EXP_STORE).days?.[date]||{};if(exp.situation?.done)submitDaily('situation',exp.situation.score||0,force);
      const lc=readStore(LASTCALL_STORE).days?.[date];if(lc?.done)submitDaily('lastcall',lc.score||0,force);
    })
  }

  function bindScoreObservers(){
    const targets=['#todayTotal','#situationStatus','#situationScore','#lastCallCardStatus','#lastCallScore'].map($).filter(Boolean);if(targets.length){const o=new MutationObserver(()=>syncDailyScores());targets.forEach(t=>o.observe(t,{subtree:true,childList:true,characterData:true,attributes:true}))}
  }
