(()=>{
  ...
  function pathFor(pts){if(!pts.length)return'';return'M '+pts[0].x+' '+pts[0].y+pts.slice(1).map((p,i)=>{const a=pts[i],distance=Math.hypot(p.x-a.x,p.y-a.y),sag=Math.min(1.45,.3+distance*.012);return' Q '+((a.x+p.x)/2)+' '+((a.y+p.y)/2+sag)+' '+p.x+' '+p.y}).join('')}
  function draw(){syncOrder();const pts=points(),d=pathFor(pts);yarn.forEach(el=>el.setAttribute('d',d));pins.innerHTML=pts.map(p=>`<circle class="knot-shadow" cx="${p.x+.28}" cy="${p.y+.38}" r="1.48"/><circle class="knot" cx="${p.x}" cy="${p.y}" r="1.25"/><circle class="knot-light" cx="${p.x-.32}" cy="${p.y-.36}" r=".3"/>`).join('');updateBest()}
