(()=>{
  ...
  Promise.all(Array.from({length:4},(_,i)=>fetch(`./live-${String(i).padStart(2,'0')}.b64`).then(r=>{if(!r.ok)throw Error(r.status);return r.text()}))).then(parts=>{world.style.backgroundImage=`url(data:image/webp;base64,${parts.join('')})`;world.classList.add('ready')}).catch(()=>document.querySelector('.loading').textContent='PRINT SHOP ART FAILED TO LOAD');
  for(const letters of ['QWERTYUIOP','ASDFGHJKL','ZXCVBNM']){const line=document.createElement('div');line.className='key-row';for(const ch of letters){const b=document.createElement('button'),label=document.createElement('span');b.className='key';label.textContent=ch;b.appendChild(label);b.dataset.key=ch;b.onclick=()=>type(ch);line.appendChild(b)}keyboard.appendChild(line)}
