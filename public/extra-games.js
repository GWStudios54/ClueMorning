(()=>{
  ...
  function style(){
    if(document.querySelector('#extraGamesStyle'))return;
    const s=document.createElement('style');s.id='extraGamesStyle';s.textContent=`
  ...
  function cardFor(game){
    let b=document.querySelector(`[data-extra-game="${game.id}"]`);
    if(b)return b;
    b=document.createElement('button');
    b.type='button';
    b.className=`game-card extra-game-home-card ${game.id}-home-card`;
    b.dataset.extraGame=game.id;
    b.dataset.morePlayHome='1';
    ...
    b.addEventListener('click',()=>{location.href=game.href});
    return b;
  }

  function sync(){
    style();
    const more=document.querySelector('#morePlaySection .more-play-grid');
    const home=document.querySelector('#today .game-cards');
    const target=more||home;
    if(!target)return;

    const heading=document.querySelector('#morePlaySection .more-play-heading p');
    const headingCopy="The daily set ends. These don't.";
    if(heading&&heading.textContent!==headingCopy)heading.textContent=headingCopy;
