...
async function dailyResponse(response){
  try{
    const data=await response.clone().json(),date=data.date||pacificDateKey();
    delete data.lockbox;
    data.lastcall=publicLastCall(date);
    data.dailyGames=8;
    return json(data,response.status);
  }catch{return response}
}
...
async function healthResponse(response){
  try{
    const data=await response.clone().json();
    data.dailyGames=8;
    data.lastCallRounds=LAST_CALL_ROUNDS;
    delete data.lockbox;
    return json(data,response.status);
  }catch{return response}
}
...
export default {
  async fetch(request,env){
    ...
    if(request.method==="POST"&&path==="/api/lastcall/check"){
      const b=await body(request),date=requestDate(request,b),result=checkLastCall(date,b.optionId);
      if(!result)return json({error:'That choice is not part of today’s Last Call.'},400);
      return json({ok:true,...result});
    }
    if(request.method==="POST"&&path==="/api/lastcall/reveal"){
      const b=await body(request),date=requestDate(request,b);
      if(b.finished!==true)return json({error:'Finish or bank the round before revealing the board.'},403);
      return json({ok:true,options:revealLastCall(date)});
    }
